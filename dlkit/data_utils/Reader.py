...
class KonDataset(data.Dataset):
    ...
    def __init__(self, list_file, train, transform):
        ...
        for line in lines:
            ...
            y2 = float(splited[4])
            c = splited[5]
            # print(c, int(c)+1, torch.tensor(int(c) + 1, dtype=torch.long))
            self.boxes.append(torch.Tensor([x, y, x2, y2]))
            self.labels.append(torch.tensor(int(c)+1, dtype=torch.long))
    ...
    def __getitem__(self, idx):
        ...
        y, h = int(boxes[1]), int(boxes[3])
        x, w = int(boxes[0]), int(boxes[2])
        img = img[y:y+h, x:x+w, :]
...
class KonYOLODataset(data.Dataset):
    image_size = 448
    ...
    def randomBlur(self, bgr):
        # bgr = torch.squeeze(bgr, dim=0)
        # if bgr.shape[2] == 3:
        #     print("bgr size before:", bgr.shape)
        bgr = bgr.transpose(1, 2, 0)
        if random.random() < 0.5:
            bgr = cv2.blur(bgr, (5, 5))
        # bgr = torch.unsqueeze(bgr, dim=0)
        # if bgr.shape[0] == 3:
        bgr = bgr.transpose(2, 0, 1)
            # print("bgr size after:", bgr.shape)
        return bgr
...
def main():
    ...
    import torchvision.transforms as transforms
    import matplotlib.pyplot as plt
    train_dataset = KonDataset(list_file='./konface_train_val_list_single.txt', train=True,
                                transform=None)
