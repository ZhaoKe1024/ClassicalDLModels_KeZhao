...
class Generator_conv(nn.Module):
    ...
    def __init__(self, nc=3, conv_dim=64, repeat_num=2):
        ...
        self.main = nn.Sequential(*self.start_layers, *self.down_layers, *self.bottle_encoder_layers,
                                  *self.bottle_decoder_layers, *self.up_layers)
...
class Generator_fc(nn.Module):
    ...
    def __init__(self, nc=3, conv_dim=64, repeat_num=2, z_dim=500):
        ...
        for i in range(4):
            if i <= 1:
                self.up_layers.append(
                    nn.ConvTranspose2d(curr_dim, curr_dim, kernel_size=4, stride=2, padding=1, bias=False))
